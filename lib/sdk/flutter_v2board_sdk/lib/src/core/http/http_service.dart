...
class HttpService {
  ...
  void _initializeDio() {
    _dio = Dio(BaseOptions(
      ...
      headers: {
        ...
        'User-Agent': httpConfig.userAgent ?? 'Orange-V2Board-SDK/1.0',
      },
    ));

    // 配置客户端证书和SSL验证
    (_dio.httpClientAdapter as IOHttpClientAdapter).createHttpClient = () {
      SdkLogger.d('[V2BoardSDK] 🔨 创建 HttpClient...');
      final client = HttpClient();

      // 配置代理
      if (httpConfig.proxyUrl != null && httpConfig.proxyUrl!.isNotEmpty) {
        SdkLogger.d('[V2BoardSDK] 🔌 配置代理: ${httpConfig.proxyUrl}');

        final proxyConfig = _parseProxyConfig(httpConfig.proxyUrl!);
        SdkLogger.d(
            '[V2BoardSDK] 🔄 解析: host=${proxyConfig['host']}, port=${proxyConfig['port']}, auth=${proxyConfig['username'] != null}');
        ...
        SdkLogger.i('[V2BoardSDK] ✅ SOCKS5 代理配置完成');
      }
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      onRequest: (options, handler) {
        ...
        SdkLogger.d(
            '[V2BoardSDK] 📡 ${options.method} $fullUrl | proxy: $proxyStatus ($proxyInfo)');
  ...
  Map<String, dynamic> _normalizeResponse(dynamic responseData) {
    ...
    if (jsonResponse.containsKey('status')) {
      ...
      // V2Board normally returns `{data: ...}` and some endpoints also expose
      // top-level fields such as checkout's `type`. Preserve that wire shape;
      // wrapping the whole map under another `data` key breaks every adapter.
      return {
        'success': true,
        ...jsonResponse,
      };
