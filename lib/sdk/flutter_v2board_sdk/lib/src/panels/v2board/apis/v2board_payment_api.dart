...
class V2BoardPaymentApi {
  ...
  Future<ApiResponse<List<PaymentMethodInfo>>> getPaymentMethods() async {
    try {
      final result =
          await _httpService.getRequest('/api/v1/user/order/getPaymentMethod');

      if (result['data'] != null) {
        final List<dynamic> methodsJson = result['data'] as List;
        final methods = methodsJson
            .map((json) =>
                PaymentMethodInfo.fromJson(json as Map<String, dynamic>))
            .toList();
  ...
  Future<ApiResponse<dynamic>> submitOrderPayment(
  ...
  Future<ApiResponse<PaymentStatusResult>> checkPaymentStatus(
    ...
    try {
      ...
      final statusCode = result['data'] as int?;
      final status = switch (statusCode) {
        3 => PaymentStatusResult.success(result['message'] as String?),
        2 => PaymentStatusResult.canceled(result['message'] as String?),
        0 || 1 => PaymentStatusResult.pending(result['message'] as String?),
        _ => PaymentStatusResult.failed(result['message'] as String?),
      };
      return ApiResponse(
        success: true,
        data: status,
