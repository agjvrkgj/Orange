...
/// V2Board client entry point.
class V2BoardSDK {
  static V2BoardSDK? _instance;
  static V2BoardSDK get instance => _instance ??= V2BoardSDK._internal();

  V2BoardSDK._internal();
  ...
  Future<void> initialize(
    ...
    _isInitialized = true;
    SdkLogger.i('V2BoardSDK initialized with panel type: $_panelType');
  }

  /// Saves V2Board's JWT `auth_data` exactly as returned by the server.
  ///
  /// V2Board does not use the Bearer authentication scheme. Its user
  /// middleware decrypts the raw `Authorization` header as a JWT.
  ...
  void dispose() {
    if (_isInitialized) {
      ...
      SdkLogger.i('V2BoardSDK disposed');
    }
  }
  ...
  void _checkInitialized() {
    if (!_isInitialized) {
      throw ConfigException(
          'V2BoardSDK is not initialized. Call initialize() first.');
    }
  }
}
